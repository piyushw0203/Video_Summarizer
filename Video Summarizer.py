...
ydl_opts = {
    ...
    'outtmpl' : "E:/College/SY/EDI Sem 3/edi" , 
}
...
def speech_2_text(apikey , url , link):
    if link is None:
        ...
        with open('E:/College/SY/EDI Sem 3/edi.mp3', 'rb') as f:
...
def abstractive(text , ext_summary , link):
    ...
    if(size<=5700000):
        ...
        abs_summary=summarizer(text,min_length=20,max_length=200)
...
def roguer(text,abs_summary,link):
    if link is None:
        return ''
    scorer = rouge_scorer.RougeScorer(['rougeL'], use_stemmer=True)
    scores = scorer.score(text,abs_summary)
    return scores
...
def rogue(link,scores):
    if link is None:
        return ''
    my_expander = st.expander(label="Accuracy Score")
    my_expander.write(scores)
...
file = Path() / 'E:/College/SY/EDI Sem 3/edi.mp3'  # or Path('./doc.txt')
...
roguee=roguer(text,abs_summary,link)
rogue(link,roguee)
